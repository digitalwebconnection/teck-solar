...
export default function QuoteModal() {
    ...
    return (<div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      {/* Backdrop */}
      <div className={`absolute inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity duration-300 ${isAnimatingOut ? "opacity-0" : "opacity-100"}`}/>

      {/* Modal Container */}
      <div className={`relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row transition-all duration-300 ${isAnimatingOut
        ...
        <div className="hidden md:flex md:w-5/12 bg-slate-900 relative p-10 flex-col justify-between overflow-hidden">
          {/* Background Image */}
          <div className="absolute inset-0 bg-cover bg-center opacity-40 mix-blend-overlay" style={{ backgroundImage: "url('/images/hero-residential.jpg')" }}/>
        ...
        ].map((benefit, i) => (<li key={i} className="flex items-center gap-3 text-white/90 font-medium">
        ...
        <div className="w-full md:w-7/12 p-6 sm:p-8 relative">
          {/* Close Button */}
          <button type="button" onClick={handleClose} className="absolute top-4 right-4 sm:top-6 sm:right-6 w-10 h-10 rounded-full bg-slate-50 hover:bg-slate-100 text-slate-400 hover:text-slate-600 flex items-center justify-center transition-colors focus:outline-none z-50 cursor-pointer">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/>
            </svg>
          </button>

          {isSuccess ? (<div className="flex flex-col items-center justify-center h-full text-center py-12 px-6 animate-fade-in">
              ...
              <button type="button" onClick={handleClose} className="px-10 py-3.5 rounded-xl font-heading font-bold text-white bg-slate-900 hover:bg-slate-800 transition-colors duration-300 cursor-pointer">
                Close Window
              </button>
