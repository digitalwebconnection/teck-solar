...
export default function OfficeLocationsSection() {
  ...
  return (
    <section className="py-16 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 bg-navy-50" ref={officesReveal.ref}>
      <div className="max-w-7xl mx-auto w-full">
        <div className={`text-center mb-14 transition-all duration-700 ${officesReveal.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <span className="text-primary-500 font-heading font-semibold text-sm tracking-widest uppercase">Our Locations</span>
          <h2 className="text-3xl md:text-4xl font-heading font-bold text-navy-900 mt-3">Office Locations</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {offices.map((office, i) => (
            ...
            >
              ...
              <h3 className="text-lg font-heading font-semibold text-navy-900">{office.city}</h3>
              <p className="text-navy-500 text-sm mt-2">{office.address}</p>
              <div className="mt-4 space-y-2">
                <a href={`tel:${office.phone.replace(/\s/g, '')}`} className="flex items-center gap-2 text-sm text-navy-600 hover:text-primary-500 transition-colors">
                ...
                <a href={`mailto:${office.email}`} className="flex items-center gap-2 text-sm text-navy-600 hover:text-primary-500 transition-colors">
