...
function RenderDropdownIcon({ icon, className = "w-5 h-5" }) {
    switch (icon) {
        case "home":
...
export default function Header() {
    ...
    return (<>
      ...
      <div className="w-full bg-slate-900 text-slate-300 text-[13px] py-2.5">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between px-4 w-full gap-y-2">
          ...
          <div className="flex items-center gap-6">
            {/* Phone */}
            <a href="tel:1300000832" className="flex items-center gap-2 font-medium text-white hover:text-primary-400 transition-colors group">
            ...
            {/* Email */}
            <a href="mailto:info@tecksolar.com.au" className="hidden sm:flex items-center gap-2 font-medium hover:text-white transition-colors">
          ...
          <div className="flex items-center gap-5 ml-auto">
            ...
            {/* WhatsApp */}
            <a href="https://wa.me/61400000000" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 font-medium hover:text-green-400 transition-colors group">
      ...
      <header className={`sticky top-0 z-50 w-full bg-white border-b border-slate-200 transition-shadow duration-200 ${scrolled ? "shadow-md shadow-slate-900/8" : "shadow-xs"}`}>
        <div className="max-w-7xl mx-auto px-4 w-full">
          <div className="flex items-center justify-between h-20">
            {/* Brand Logo - Crisp on White */}
            <Link to="/" className="flex items-center py-2 shrink-0 group relative z-50" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
              <img src={logo} alt="Teck Solar" className="h-14 md:h-16 max-h-16 w-auto object-contain transition-transform duration-200 group-hover:scale-[1.02]"/>
            </Link>
            ...
            return (<div key={item.label} className="relative nav-item" onMouseEnter={() => item.children && setOpenDropdown(item.label)} onMouseLeave={() => setOpenDropdown(null)}>
                    <Link to={item.children ? "#" : item.path} onClick={(e) => item.children && e.preventDefault()} className={`px-4 py-2 rounded-full text-[15px] font-heading font-semibold transition-all duration-200 flex items-center gap-1.5
                        ${isActive || openDropdown === item.label
                    ? "text-primary-600 bg-primary-50"
                    : "text-slate-700 hover:text-primary-600 hover:bg-slate-50"}`}>
                      <span>{item.label}</span>
                      {item.children && (<svg className={`w-4 h-4 transition-transform duration-200 group-hover:text-primary-600 ${openDropdown === item.label
                    ...
                    {item.label === "Services" && item.children && (<div className={`absolute top-full left-0 pt-4 w-[680px] max-w-[calc(100vw-2rem)] transition-all duration-200 ${openDropdown === item.label
                        ...
                        return (<Link key={child.path} to={child.path} className={`group/item flex items-start gap-4 p-4 rounded-md transition-all duration-300 border ${isCurrent
                                ? colorTheme.activeCardBg
                                : `border-transparent ${colorTheme.cardHoverBg}`}`}>
                                  <div className={`w-12 h-12 rounded-md flex items-center justify-center shrink-0 border transition-all duration-300 ${isCurrent
                                ...
                                : `bg-slate-50 border-slate-200 text-slate-400 ${colorTheme.iconHoverBg} ${colorTheme.iconHoverBorder} ${colorTheme.iconHoverColor} ${colorTheme.iconHoverShadow}`}`}>
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between">
                                      <span className={`text-[15px] font-heading font-bold transition-colors ${isCurrent
                                ? colorTheme.activeText
                                : `text-slate-700 ${colorTheme.textHover}`}`}>
                                        {child.label}
                                      </span>
                                    </div>
                                    <p className={`text-[13px] mt-1 line-clamp-2 leading-relaxed transition-colors ${isCurrent ? "text-slate-600" : "text-slate-500 group-hover/item:text-slate-600"}`}>
                                      {child.desc}
                                    </p>
                                  </div>
                                </Link>);
                    ...
                    {item.label === "Resources" && item.children && (<div className={`absolute top-full left-0 pt-4 w-[680px] max-w-[calc(100vw-2rem)] transition-all duration-200 ${openDropdown === item.label
                        ? "opacity-100 visible translate-y-0"
                        : "opacity-0 invisible -translate-y-2 pointer-events-none"}`}>
                        <div className="bg-white rounded-md shadow-[0_10px_40px_-10px_rgba(0,0,0,0.15)] border border-slate-200 p-6 overflow-hidden">
                          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-3 xl:gap-4">
              ...
              <button type="button" className="lg:hidden p-2.5 rounded-xl bg-slate-100 text-slate-700 hover:bg-slate-200 hover:text-primary-600 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500/40" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Toggle navigation menu">
                {mobileOpen ? (<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className={`lg:hidden fixed inset-0 z-50 transition-all duration-300 ${mobileOpen
            ? "visible pointer-events-auto"
            : "invisible pointer-events-none"}`}>
        {/* Backdrop */}
        <div className={`absolute inset-0 bg-slate-900/35 backdrop-blur-xs transition-opacity duration-300 ${mobileOpen ? "opacity-100" : "opacity-0"}`} onClick={() => setMobileOpen(false)}/>

        {/* Drawer Content */}
        <div className={`absolute right-0 top-0 h-full w-84 max-w-[85vw] bg-white border-l border-slate-200 shadow-2xl flex flex-col transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "translate-x-full"}`}>
          {/* Drawer Top Header */}
          <div className="p-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
            <Link to="/" onClick={() => setMobileOpen(false)} className="flex items-center">
              <img src={logo} alt="Teck Solar" className="h-10 w-auto object-contain"/>
            </Link>
            <button type="button" className="p-2 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-slate-200/60 transition-colors" onClick={() => setMobileOpen(false)} aria-label="Close menu">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-1">
            ...
            if (item.children) {
                ...
                return (<div key={item.label} className="border-b border-slate-100 pb-1 mb-1">
                    <button type="button" className={`w-full flex items-center justify-between px-3 py-3 rounded-xl text-sm font-heading font-semibold transition-colors ${isActive || isOpen
                        ? "text-primary-600 bg-primary-50"
                        : "text-slate-700 hover:text-primary-600 hover:bg-slate-50"}`} onClick={() => setOpenDropdown(isOpen ? null : item.label)}>
                      <span>{item.label}</span>
                      <svg className={`w-4 h-4 transition-transform duration-200 ${isOpen ? "rotate-180 text-primary-600" : "text-slate-400"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/>
                      </svg>
                    </button>

                    <div className={`overflow-hidden transition-all duration-200 space-y-1 pl-2 pt-1 ${isOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
                      {item.children.map((child) => {
                        const isCurrent = location.pathname === child.path;
                        return (<Link key={child.path} to={child.path} className={`flex items-start gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors ${isCurrent
            ...
            return (<Link key={item.label} to={item.path} className={`block px-3 py-3 rounded-xl text-sm font-heading font-semibold transition-colors ${isActive
          ...
          <div className="p-4 border-t border-slate-200 bg-slate-50 space-y-2.5">
            <Button onClick={() => { openModal(); setMobileOpen(false); }} className="w-full">
              Get a Free Quote
            </Button>

            <div className="grid grid-cols-2 gap-2 text-center text-xs">
              <a href="tel:1300000832" className="p-2.5 rounded-xl bg-white border border-slate-200 text-slate-800 hover:border-primary-500 transition-colors flex flex-col items-center gap-1 shadow-xs font-semibold">
                <svg className="w-4 h-4 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                </svg>
                <span>Call 1300</span>
              ...
              <a href="https://wa.me/61400000000" target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-xl bg-white border border-slate-200 text-slate-800 hover:border-eco-500 transition-colors flex flex-col items-center gap-1 shadow-xs font-semibold">
