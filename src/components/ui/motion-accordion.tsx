...
function AccordionItem({
  ...
  return (
    <motion.div
      layout
      className={cn(
        "overflow-hidden rounded-[30px] bg-slate-50 border border-slate-200 text-slate-900 shadow-sm",
        isOpen && "bg-white border-brand-blue-200 shadow-md",
      )}
    ...
    >
      ...
      >
        <span className="text-[clamp(1.1rem,1.4vw,1.2rem)] font-heading font-semibold tracking-tight leading-snug">
          {item.question}
        </span>
        ...
        <motion.span
          ...
          className={cn("inline-flex size-10 shrink-0 items-center justify-center rounded-full transition-colors duration-300", isOpen ? "bg-brand-blue-50 text-brand-blue-600" : "bg-slate-100 text-slate-500")}
        >
          {isOpen ? (
            <svg
              width="14"
      ...
      >
        <motion.div
          ...
          className="px-7 pb-7"
        >
          <p className="text-base leading-relaxed font-light text-slate-600">
            {item.answer}
          </p>
        </motion.div>
      </motion.div>
...
export function MotionAccordion({
  ...
  return (
    <div className={cn("w-full max-w-3xl mx-auto", className)}>
      <div className="flex flex-col rounded-[34px]" style={{ gap }}>
        {items.map((item, i) => (
          <AccordionItem
            key={i}
            item={item}
