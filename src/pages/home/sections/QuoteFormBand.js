...
export default function QuoteFormBand() {
    ...
    return (<section className="py-8 lg:py-12 bg-slate-50/70 relative border-t border-slate-100 overflow-hidden" ref={ref}>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10">
        {/* Section Header */}
        <div className={`text-center mb-14 lg:mb-16 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>         
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => {
            ...
            return (<div key={index} className={`bg-white rounded-2xl border transition-all duration-500 overflow-hidden ${isOpen
                ...
                <button type="button" onClick={() => toggleFaq(index)} className="w-full flex items-center justify-between p-6 sm:p-7 text-left cursor-pointer transition-colors group" aria-expanded={isOpen}>
                  <span className="font-heading font-bold text-slate-900 text-base sm:text-lg pr-4 flex items-center gap-4">
                    <span className={`w-10 h-10 rounded-xl text-sm font-extrabold flex items-center justify-center shrink-0 transition-all duration-500 shadow-sm ${isOpen
                  ...
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition-all duration-500 ${isOpen
