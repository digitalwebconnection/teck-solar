...
export default function WiFiStepsSection() {
    ...
    return (<section className="py-24 lg:py-32 bg-slate-50 relative z-20" ref={sectionReveal.ref}>
        ...
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
          ...
          <div className="lg:w-2/3 relative">
             ...
             <div className="space-y-8 lg:space-y-12">
               {steps.map((s, i) => (<div key={i} className={`relative md:pl-16 lg:pl-20 transition-all duration-700 ${sectionReveal.visible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`} style={{ transitionDelay: `${i * 100}ms` }}>
                    ...
                    <div className="bg-white rounded-2xl p-8 lg:p-10 border border-slate-200 shadow-sm hover:shadow-xl hover:shadow-[#144E9A]/5 transition-all duration-300 relative overflow-hidden group">
                      ...
                      {s.tips.length > 0 && (<div className="bg-slate-50 rounded-[1.5rem] p-6 border border-slate-100">
                          <span className="text-xs font-bold text-[#E56D00] uppercase tracking-widest block mb-4">Pro Tips</span>
                          <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {s.tips.map((tip, ti) => (<li key={ti} className="flex items-start gap-3 text-sm text-slate-600">
