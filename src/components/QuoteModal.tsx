...
export default function QuoteModal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await submitToWeb3Forms(
      {
        ...
        quarterly_bill: formData.quarterlyBill || "Not specified",
        page: "Teck Solar Free Quote Popup",
      },
      {
        subject: `New Free Quote Request - ${fullName} (${formData.postcode})`,
        from_name: "Teck Solar Quote Modal",
      }
    );
    ...
    if (res.success) {
      ...
      setErrorMessage(
        res.message || "Failed to submit quote request. Please try again."
      );
    }
  };
        ...
        <div className="hidden md:flex md:w-5/12 bg-slate-900 relative p-10 flex-col justify-between overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-300 font-light leading-relaxed mb-8">
              Join thousands of Australians saving up to 80% on their energy
              bills. Get a customized, no-obligation quote from our
              SAA-accredited experts.
            </p>
          ...
          <div className="relative z-10 mt-12 bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-5">
            ...
            <p className="text-white/80 text-sm italic mb-2">
              "Teck Solar cut our power bills by 85%. Flawless process from
              start to finish."
            </p>
          ...
          {isSuccess ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-12 px-6 animate-fade-in">
              ...
              <p className="text-slate-600 mb-8 max-w-sm mx-auto leading-relaxed">
                Thank you for your interest. One of our solar experts will
            ...
            <div className="animate-fade-in">
              <div className="mb-6 pr-12">
                <h2 className="text-xl sm:text-2xl font-heading font-extrabold text-slate-900 mb-1.5">
                  Get Your Free Quote
                </h2>
              ...
              {errorMessage && (
                <div className="mb-4 p-3.5 rounded-xl text-sm font-medium bg-red-50 text-red-800 border border-red-200 animate-fade-in">
                  {errorMessage}
                </div>
              )}
              ...
              <form className="space-y-3 sm:space-y-4" onSubmit={handleSubmit}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      minLength={4}
                      pattern="^[\d]+$"
                      title="Please enter a valid numerical postcode"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="block text-[13px] font-semibold text-slate-700">
                    ...
                    <select
                      ...
                      required
                      className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-blue-500/20 focus:border-brand-blue-500 outline-none transition-all text-slate-700 text-sm"
                    >
                ...
                <div className="space-y-1">
                  <label className="block text-[13px] font-semibold text-slate-700">
                    Average Quarterly Bill (Optional)
                  </label>
                  <select
                    name="quarterlyBill"
                    value={formData.quarterlyBill}
                    onChange={(e) =>
                      setFormData({ ...formData, quarterlyBill: e.target.value })
                    }
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-blue-500/20 focus:border-brand-blue-500 outline-none transition-all text-slate-700 text-sm"
                  >
                ...
                <p className="text-center text-[11px] text-slate-400 mt-3">
                  Your information is secure. We'll only use it to provide your
                  quote.
                </p>
              </form>
            </div>
