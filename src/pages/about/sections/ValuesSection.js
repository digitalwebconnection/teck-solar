...
const values = [
    {
        ...
        description: "We are committed to reducing carbon emissions and accelerating Australia's transition to clean, dependable renewable energy.",
    ...
    },
    {
        ...
        description: "Every solar system is tailored to your energy profile. We provide transparent advice from initial assessment through lifetime support.",
        icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
        ...
        description: "We partner exclusively with tier-1 Bloomberg-rated manufacturers and execute installations that exceed Australian Standards AS/NZS 5033.",
        icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
        ...
        title: "Innovation",
        description: "We integrate smart WiFi-connected inverters, modular battery storage, and dynamic EV charging for total household energy management.",
        icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function ValueTiltCard({ item, index, visible, }) {
    ...
    return (<div ref={cardRef} onMouseEnter={() => setIsHovered(true)} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} style={{
            transform: isHovered
        ...
        }} className={`group relative overflow-hidden rounded-2xl bg-white p-6 border transition-all duration-300 flex flex-col justify-between select-none cursor-pointer ${isHovered
            ? "border-brand-blue-500 shadow-2xl shadow-brand-blue-900/15"
            : "border-slate-200/80 shadow-xs hover:border-slate-300"} ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
      {/* 1. Dynamic Rotating Conic Laser Border Beam on Active Hover */}
      {isHovered && (<div className="absolute inset-0 rounded-2xl pointer-events-none p-[1.5px] overflow-hidden">
      ...
      <div className="mt-6 pt-4 border-t border-slate-100 relative z-10 flex items-center justify-between" style={{ transform: "translateZ(18px)" }}>
...
export default function ValuesSection() {
    ...
    return (<section className="py-14 sm:py-18 lg:py-22 px-4 sm:px-6 lg:px-8 bg-brand-blue-50/50 relative overflow-hidden border-b border-slate-100" ref={ref}>
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        ...
        <div className={`max-w-3xl transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
          {values.map((val, i) => (<ValueTiltCard key={val.step} item={val} index={i} visible={visible}/>))}
        </div>
      </div>
    </section>);
}
