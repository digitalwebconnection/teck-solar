...
export default function OfficeLocationsSection() {
    ...
    return (<section className="py-8 lg:py-12 px-4 sm:px-6 lg:px-8 bg-white border-t border-slate-200" ref={officesReveal.ref}>
      <div className="max-w-[1600px] mx-auto w-full">
        <div className={`mb-24 max-w-4xl mx-auto text-center transition-all duration-1000 ${officesReveal.visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        ...
        <div className={`flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-slate-200 border-y border-slate-200 transition-all duration-1000 delay-300 ${officesReveal.visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}>
          {offices.map((office, i) => (<div key={i} className="flex-1 py-16 lg:px-16 relative group overflow-hidden first:pl-0 last:pr-0 lg:first:pl-8 lg:last:pr-8">
              {/* Massive Watermark */}
