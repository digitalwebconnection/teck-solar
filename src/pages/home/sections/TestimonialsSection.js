...
const testimonials = [
    {
        ...
        rating: 5,
        initials: "SM",
        quote: "Teck Solar transformed our electricity bills completely. From the initial consultation to final commissioning, the team was punctual and respectful of our home. Our quarterly power bill dropped by over 80%!",
    },
    {
        name: "James Chen",
        ...
        rating: 5,
        initials: "JC",
        quote: "As a warehouse operator, spiralling grid electricity was hurting our margins. Teck Solar engineered a high-yield system that delivered on every performance projection. Exceptional service and zero downtime.",
    },
    {
        ...
        rating: 5,
        initials: "DT",
        quote: "The quality of Tier-1 equipment and flawless installation on our tile roof was outstanding. We now power both our family home and electric vehicle almost entirely with clean Australian sunshine.",
    },
    {
        ...
        rating: 5,
        initials: "MC",
        quote: "Living rurally, reliability is everything. Teck Solar set up an off-grid capable battery system that has kept our power running through multiple regional grid blackouts without missing a beat.",
    },
    {
        ...
        rating: 5,
        initials: "OR",
        quote: "From paperwork and Western Power grid approvals to rooftop panel placement, Teck Solar managed every single step seamlessly. Our daytime commercial electricity bills have literally plummeted.",
    },
    {
        ...
        rating: 5,
        initials: "LW",
        quote: "The aesthetic of the all-black panels blends seamlessly with our roof design. The mobile monitoring app is brilliant — we can track solar generation and household consumption in real-time every day!",
    },
];
export default function TestimonialsSection() {
    ...
    return (<section className="py-16 sm:py-20 lg:py-14 px-4 sm:px-6 lg:px-8 bg-slate-50/60 border-y border-slate-100/70 relative overflow-hidden" ref={ref}>
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        ...
        <div className={`flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 sm:mb-14 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          ...
          <div className="flex items-center gap-3 self-start md:self-end">
            <button type="button" onClick={prevSlide} aria-label="Previous testimonial" className="w-11 h-11 rounded-full border border-slate-200 bg-white hover:bg-slate-50 hover:border-[#E56D00] text-slate-700 hover:text-[#E56D00] flex items-center justify-center shadow-sm hover:shadow-md transition-all cursor-pointer">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.2} d="M15 19l-7-7 7-7"/>
              </svg>
            </button>
            <button type="button" onClick={nextSlide} aria-label="Next testimonial" className="w-11 h-11 rounded-full border border-slate-200 bg-white hover:bg-slate-50 hover:border-[#E56D00] text-slate-700 hover:text-[#E56D00] flex items-center justify-center shadow-sm hover:shadow-md transition-all cursor-pointer">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="overflow-hidden py-4 -my-4" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
          <div className="flex transition-transform duration-700 ease-out py-4" style={{
            ...
            return (<div key={i} className="w-full md:w-1/2 lg:w-1/3 flex-shrink-0 px-3 sm:px-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <div className="flex text-amber-400 flex-shrink-0">
                              {[...Array(t.rating)].map((_, idx) => (<svg key={idx} className="w-3.5 h-3.5 fill-current" viewBox="0 0 20 20">
        ...
        <div className="flex items-center justify-center gap-2 mt-10">
          {[...Array(totalDots)].map((_, idx) => (<button key={idx} type="button" onClick={() => setCurrent(idx)} className={`transition-all duration-300 rounded-full cursor-pointer ${current === idx
                ? "w-8 h-2.5 bg-gradient-to-r from-[#144E9A] to-brand-blue-400"
                : "w-2.5 h-2.5 bg-slate-200 hover:bg-slate-300"}`} aria-label={`Go to slide ${idx + 1}`}/>))}
        </div>
      </div>
    </section>);
}
