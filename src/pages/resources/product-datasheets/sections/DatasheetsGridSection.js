...
export default function DatasheetsGridSection() {
    ...
    return (<section className="w-full pb-32 animate-slide-up" ref={gridReveal.ref} style={{ animationDelay: '200ms' }}>
      ...
      <div className="flex flex-col lg:flex-row gap-12 lg:gap-24 items-start relative mt-12 lg:mt-0">
        ...
        <aside className="w-full lg:w-1/4 shrink-0 lg:sticky lg:top-12 border-b lg:border-b-0 border-slate-200 pb-8 lg:pb-0">
          ...
          <div className="relative mb-12 lg:mb-16">
            <input type="text" placeholder="Search products or codes..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-transparent text-lg font-medium text-slate-900 placeholder-slate-500 border-b-2 border-slate-300 focus:border-slate-900 focus:outline-none pb-3 transition-colors pr-8"/>
            {searchQuery ? (<button onClick={() => setSearchQuery('')} className="absolute right-0 top-1 text-slate-900 hover:text-amber-500 transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
              </button>) : (<div className="absolute right-0 top-1 text-slate-500 pointer-events-none">
          ...
          <nav className="flex flex-row overflow-x-auto lg:overflow-visible lg:flex-col gap-6 lg:gap-4 pb-4 lg:pb-0 no-scrollbar">
            {categories.map((cat) => (<button key={cat} onClick={() => setActiveCategory(cat)} className={`text-left text-xl lg:text-3xl font-heading font-black tracking-tight whitespace-nowrap transition-all duration-300  cursor-pointer ${activeCategory === cat
        ...
        <div className="w-full lg:w-3/4">
          ...
          {filteredDatasheets.length > 0 ? (<div className="flex flex-col">
              {filteredDatasheets.map((ds) => (<div key={ds.name} className="group relative border-b border-slate-200 hover:border-slate-900 transition-colors duration-500 overflow-hidden">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 px-4 py-8 lg:py-12 transition-colors duration-500">
                    ...
                    <button type="button" aria-label={`Download PDF for ${ds.name}`} className="shrink-0 flex items-center gap-4 text-slate-300 group-hover:text-amber-500 transition-all duration-500 cursor-pointer">
            ...
            </div>) : (<div className="w-full py-32 flex flex-col items-start border-b border-slate-200">
