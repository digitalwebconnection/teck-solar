...
export default function Footer() {
    return (<footer className="bg-slate-900 border-t border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-4">
          ...
          <div className="space-y-6">
            <Link to="/" className="inline-block" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
              <img src={logo} alt="Teck Solar" className="h-12 md:h-26 w-auto object-contain"/>
            </Link>

            {/* New Energy Tech Consumer Code Badge */}
            <div className="flex items-center ">
              <img src={newEnergyTechLogo} alt="New Energy Tech Consumer Code Approved" className="w-14 h-14 object-contain shrink-0"/>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {quickLinks.map((link) => (<li key={link.path}>
                  <Link to={link.path} className="text-sm text-slate-400 hover:text-primary-600 transition-colors duration-150 inline-block">
                    {link.label}
                  </Link>
                </li>))}
            </ul>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {serviceLinks.map((link) => (<li key={link.path}>
                  <Link to={link.path} className="text-sm text-slate-400 hover:text-primary-600 transition-colors duration-150 inline-block">
                    {link.label}
                  </Link>
                </li>))}
          ...
          <div>
            ...
            <div className="space-y-3.5 text-sm">
              ...
              <div className="space-y-2.5">
                {officeLocations.map((address, index) => (<div key={index} className="flex items-start gap-2.5 text-slate-400">
              ...
              <div className="flex items-center gap-2.5 pt-2 border-t border-slate-100">
                ...
                <a href="tel:1300134077" className="text-sm font-semibold text-slate-400 hover:text-primary-600 transition-colors">
                  1300 134 077
                </a>
              </div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <a href="mailto:info@teck-solar.com.au" className="text-sm font-medium text-slate-400 hover:text-primary-600 transition-colors">
